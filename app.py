import streamlit as stimport pandas as pdimport numpy as npimport torchimport torch.nn as nnimport joblibimport plotly.express as pximport plotly.graph_objects as go
...
st.set_page_config(page_title="Bike Demand Prediction System",page_icon="🚲",layout="wide")
...
X_scaler = joblib.load("x_scaler.pkl")y_scaler = joblib.load("y_scaler.pkl")
...
class PositionalEncoding(nn.Module):def init(self, d_model, max_len=500):super().init()
...
class TransformerModel(nn.Module):def init(self, input_dim, d_model=128, nhead=8, num_layers=3):
...
page = st.sidebar.radio("Navigation",["Dashboard","Data Insights","Prediction","Dataset Explorer","Model Performance","About"])
